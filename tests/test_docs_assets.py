...
class DocsAssetVersionTests(unittest.TestCase):
    ...
    def test_idle_ui_avoids_persistent_compositor_work(self):
        ...
        experience = (DOCS / "experience.css").read_text(encoding="utf-8")
        all_css = "\n".join([styles, experience, motion_css, performance])
        self.assertNotIn("infinite", motion_css)
        self.assertNotIn("data-magnetic", motion)
        self.assertNotIn("pointermove", motion)
        self.assertNotIn("motion-particle", all_css)
        self.assertNotIn("sheet-product-float", all_css)
        self.assertNotIn("sheet-identity-float", all_css)
        self.assertNotIn("--stage-", all_css)
        self.assertNotIn("--card-", all_css)
